...
// @downloadURL https://raw.githubusercontent.com/SeRi0uS007/Steam-Economy-Enhancer/master/code.user.js
// @updateURL   https://raw.githubusercontent.com/SeRi0uS007/Steam-Economy-Enhancer/master/code.user.js
// ==/UserScript==
...
(function ($, async) {
    ...
    const SETTING_PRICE_MIN_IGNORING_PRICE_LIST = 'SETTING_PRICE_MIN_IGNORING_PRICE_LIST';
    ...
    const SETTING_SOUND_EFFECT = 'SETTING_SOUND_EFFECT'
    ...
    var settingDefaults = {
        ...
        SETTING_PRICE_MIN_IGNORING_PRICE_LIST: 0.00,
        ...
        SETTING_SOUND_EFFECT: 'https://freesound.org/data/previews/391/391539_5235550-lq.mp3'
    };
    ...
    class SoundEffect {
        ...
        play() {
            return new Promise(resolve => {
                if (this._src === 'null') return resolve();
    ...
    const soundEffect = new SoundEffect(getSettingWithDefault(SETTING_SOUND_EFFECT))
    ...
    if (currentPage == PAGE_INVENTORY) {
        ...
        var sellQueue = async.queue(function (task, next) {
            if (market.getPriceIncludingFees(task.sellPrice) <= getSettingWithDefault(SETTING_PRICE_MIN_IGNORING_PRICE_LIST) * 100) {
        ...
        function updateInventoryUI(isOwnInventory) {
            ...
            $('#see_settings').remove();
            $('#global_action_menu')
                .prepend('<span id="see_settings"><a href="javascript:void(0)">\u{1F4BE} Steam Economy Enhancer</a></span>');
    ...
    if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
        ...
        function marketListingsQueueWorker(listing, ignoreErrors, callback) {
            ...
            if (price <= getSettingWithDefault(SETTING_PRICE_MIN_IGNORING_PRICE) * 100) {
                ...
                //   console.log(marketCurrentPriceCheck, totalMarketLists);
                //   if (marketCurrentPriceCheck === totalMarketLists) {
                //       soundEffect.play();
                //   }
        ...
        function initializeMarketUI() {
            ...
            $('#see_settings').remove();
            $('#global_action_menu').prepend('<span id="see_settings"><a href="javascript:void(0)">\u{1F4BE} Steam Economy Enhancer</a></span>');
            $('#see_settings').on('click', '*', () => openSettings());
    ...
    function openSettings() {
        var price_options = $('<div id="price_options">' +
            ...
            '</div>' +
            '<div style="margin-top:6px;">' +
            'Don\'t check market listings with prices of and below:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' + SETTING_PRICE_MIN_IGNORING_PRICE + '" value=' + getSettingWithDefault(SETTING_PRICE_MIN_IGNORING_PRICE) + '>' +
            '<br/>' +
            '<div style="margin-top:6px;">' +
            'Don\'t list with prices of and below:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' + SETTING_PRICE_MIN_IGNORING_PRICE_LIST + '" value=' + getSettingWithDefault(SETTING_PRICE_MIN_IGNORING_PRICE_LIST) + '>' +
            '<br/>' +
            '</div>' +
            ...
            '</div>' +
            '<div style="margin-top:24px;">' +
            'Use sound effects:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white; border: transparent; width: 400px" type="text" id="' + SETTING_SOUND_EFFECT + '"value=' + getSettingWithDefault(SETTING_SOUND_EFFECT) + '>' +
            '</div>' +
            '</div>' +
            '</div>');
        ...
        var dialog = unsafeWindow.ShowConfirmDialog('Steam Economy Enhancer', price_options).done(function () {
            ...
            setSetting(SETTING_PRICE_MIN_IGNORING_PRICE_LIST, $('#' + SETTING_PRICE_MIN_IGNORING_PRICE_LIST, price_options).val());
            ...
            setSetting(SETTING_SOUND_EFFECT, $('#' + SETTING_SOUND_EFFECT, price_options).val());
